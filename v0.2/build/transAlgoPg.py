...
# from algo.substAlgo.basic import basic
from algo.transpAlgo.railfenceCipher import *
from algo.transpAlgo.columnarCipher import *
...
window.title("Classical Encryption using Transposition Techniques")
...
def encrypt():
    if userInput.get("1.0",'end-1c') == "Enter text here" or userInput.get("1.0",'end-1c') == "":
        ...
        log(f"{algoCombo.get()} Transposition Encryption Selected")
        print(f"{algoCombo.get()} Transposition Encryption Selected")

        if algoCombo.get() == "Railfence Cipher":
            inp = userInput.get("1.0",'end-1c')
            startTime = time.perf_counter_ns()
            obj = railfenceCipher(inp)
        ...
        if algoCombo.get() == "Columnar Cipher":
            inp = userInput.get("1.0",'end-1c')
            startTime = time.perf_counter_ns()
            obj = columnarCipher(inp)
            result = obj.encryptMessage()
            endTime = time.perf_counter_ns()
            textOutput.delete('1.0', END)
            result += f"\n\n\nTime taken for execution is: {str(round(endTime-startTime, 7))} nanoseconds."
...
def decrypt():
    if algoCombo.get() == "":
        ...
        log(f"{algoCombo.get()} Transposition Decryption Selected")
        print(f"{algoCombo.get()} Transposition Decryption Selected")
...
############################## Background Image #####################################
#####################################################################################
fname = PhotoImage(file = "assets/transAlgoBg.png")
...
backButton = Button(window, text = "<", font = ("Ubuntu_Mono_Bold_Italic bold", 13), fg = "white",
    height = 2, width = 7, bg = "#1C4722", command = lambda: back(), cursor = "exchange"
    )
backButton.place(x = 0, y = 0)

#User Input Text Area
userInput = Text(window, height = 9, width = 133, bg = "#363172", borderwidth = 0, 
...
#Output Text Area
textOutput = Text(window, height = 9, width = 133, bg = "#363172", borderwidth = 0, 
    fg = "white", padx = 2, pady = 2, cursor = "spider"
    )
...
encryptButton = Button(window, text = "Encrypt", font = "Ubuntu_Mono_Bold_Italic 9 bold", fg = "white",
    height = 3, width = 30, bg = "#363172", command = lambda: encrypt(), cursor = "pirate"
    )
encryptButton.place(x = 671, y = 379)

#Decrypt Button
decryptButton = Button(window, text = "Decrypt", font = "Ubuntu_Mono_Bold_Italic 9 bold", fg = "white",
    height = 3, width = 30, bg = "#363172", command = lambda: decrypt(), cursor = "pirate"
    )
...
combostyle.theme_create('combostyle', parent='alt',
                         settings = {'TCombobox':
                                     {'configure':
                                      {'selectbackground': '#363172',
                                       'fieldbackground': '#363172',
                                       'background': 'white',
                                       'borderwidth': 10,
                                       }}}
                         )
...
algoCombo['values'] = ('Columnar Cipher', 'Railfence Cipher')
