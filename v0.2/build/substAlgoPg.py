...
def decrypt():
    if algoCombo.get() == "":
        ...
        if algoCombo.get() == "Playfair Cipher":
            inp = userInput.get("1.0",'end-1c')
            startTime = time.perf_counter_ns()
            obj = playfairCipher(inp)
...
#Back Button
backButton = Button(window, text = "<", font = ("Ubuntu_Mono_Bold_Italic bold", 13), fg = "black",
    height = 2, width = 7, bg = "#8BFDFF", command = lambda: back(), cursor = "exchange"
    )
